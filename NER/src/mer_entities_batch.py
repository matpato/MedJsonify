...
def process_file_in_batch(args):
    file_path, output_dir, drugbank, vocabulary = args
...
def main(active_lexicon, input_dir, output_dir, update, drugbank_file):
    ...
    files_to_process = [(os.path.join(input_dir, filename), output_dir, drugbank, vocabulary) for filename in os.listdir(input_dir) if filename.endswith('.json')]
